...
class UpdateGuardianceRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'contact' => 'required|max:12|unique:users,contact,'.$this->guardiance->user->id,
            'sec_contact' => 'required|max:12|unique:guardiances,sec_contact,'.$this->guardiance->id,
            'guardiance_type_id' => 'required',
            'name' => 'required|string|max:255',
            'email' => 'required|email|unique:users,email,'.$this->guardiance->user->id,
            'password' => 'same:confirm-password',
            'status',
            'roles' => 'required',
            'username' => 'required|string|unique:users,username,'.$this->guardiance->user->id,
            'nic' => 'required|max:12|unique:guardiances,nic,'.$this->guardiance->id,
        ];
    }
