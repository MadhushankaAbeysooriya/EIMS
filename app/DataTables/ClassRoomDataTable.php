...
class ClassRoomDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addIndexColumn()
        ->addColumn('status', function($class_room){
            return ($class_room->status==1)?'<h5><span class="badge badge-primary">Active</span></h5>':
            '<h5><span class="badge badge-warning">Inactive</span></h5>';
        })
        ->addColumn('action', function ($class_room) {
            $encryptedId = Crypt::encrypt($class_room->id);
            $btn = '';
                $btn .= '<a href="'.route('class_rooms.edit',$encryptedId).'"
                class="btn btn-xs btn-info" data-toggle="tooltip" title="Edit">
                <i class="fa fa-pen-alt"></i> </a> ';

                $btn .= '<a href="'.route('class_rooms.show',$encryptedId).'"
                class="btn btn-xs btn-secondary" data-toggle="tooltip" title="View">
                <i class="fa fa-eye"></i> </a> ';

                if($class_room->status==1)
                {
                    $btn .='<a href="'.route('class_rooms.inactive',$encryptedId).'"
                    class="btn btn-xs btn-danger" data-toggle="tooltip"
                    title="Suspend"><i class="fa fa-trash"></i> </a> ';

                }elseif($class_room->status==0)
                {
                    $btn .='<a href="'.route('class_rooms.activate',$encryptedId).'"
                    class="btn btn-xs btn-danger" data-toggle="tooltip"
